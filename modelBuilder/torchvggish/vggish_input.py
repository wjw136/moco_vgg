...
def waveform_to_examples(data, sample_rate, return_tensor=True):
  ...
  """
    ...
    # Resample to the rate assumed by VGGish. 重新采样
    if sample_rate != vggish_params.SAMPLE_RATE:
        data = resampy.resample(data, sample_rate, vggish_params.SAMPLE_RATE)
    # print(len(data))
    # Compute log mel spectrogram features. 计算Mel spectrum(分桢 加窗 滤波)
    # num_frames, num_mel_bin
    # MFCC ceptrum 梅尔频率倒谱系数 用来分解
    # len(data) - STFT_HOP_LENGTH_SECONDS * SAMPLE_RATE // STFT_WINDOW_LENGTH_SECONDS * SAMPLE_RATE + 1
    ...
    example_window_length = vggish_params.EXAMPLE_WINDOW_FRAMES
    example_hop_length = vggish_params.EXAMPLE_HOP_FRAMES
    ...
    # num_samples，num_frames，num_bins 要多增加一维
    if return_tensor:
